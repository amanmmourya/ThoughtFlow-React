...
const Group = () => {
    ...
    const [chatWith, setChatWith] = useState(location.state?.groupName || ''); // Get chatWith from location state
    const [chatWithUser, setChatWithUser] = useState(location.state?.groupIdentity || '');
    ...
    useEffect(() => {
        const fetchMessages = async () => {
            const msg = await fetch(`${VITE_BACKEND_URL}/api/group/getMessages`, {
                method: 'POST',
                headers: {
                ...
                body: JSON.stringify({
                    ...
                    groupIdentity: chatWithUserRef.current || 'guest'
                })
            });
    ...
    useEffect(() => {
        socket.emit('registerGroupUser', user.username || 'guest');
        ...
        socket.on('receiveMessageForGroup', (sender, groupIdentity, message) => {
            if (groupIdentity == location.state.groupIdentity) {
    ...
    const handleSendMessage = async () => {
        ...
        socket.emit('newMessageSentForGroup', {
            sender: user.username || 'guest',
            groupIdentity: chatWithUserRef.current || 'guest',
            message: inputText
        });
