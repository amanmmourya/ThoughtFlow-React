...
const EnterDark = () => {
  ...
  const handleEnterGroup = async () => {
    ...
    if(groupIdentity.length<6){
        toast.error('Minimum length of identity is 6');
        return;
    }
    
    setIsLoading(true);
    const response=await fetch(`${VITE_BACKEND_URL}/api/dark/enterdark`,{
        method:'POST',
        headers:{
            'Content-Type':'application/json'
        },
        body:JSON.stringify({darkIdentity:groupIdentity})
    })
    const data=await response.json()
    if(!response.ok){
        toast.error(data.message || 'Failed to enter Group');
    }else{
        toast.success('Group entered successfully');
        navigate('/dark',{state:{
            darkIdentity:groupIdentity,
            darkName:data.darkName
        }});
    }
      ...
      <div className="w-full max-w-md">
        ...
        <div className="bg-gray-900 border border-gray-700 rounded-2xl p-8 shadow-xl">
          ...
          <div className="text-center mb-8">
            ...
            <h1 className="text-2xl font-bold text-white mb-2">Enter a Dark Group</h1>
            <p className="text-gray-400 text-sm">Enter Dark Group using dark identity</p>
          </div>
          ...
          <div className="mt-6 p-4 bg-gray-800 rounded-xl">
            <p className="text-xs text-gray-400 text-center">
              Press Enter to to navigate dark group or click the button above
            </p>
          </div>
        </div>
      </div>
